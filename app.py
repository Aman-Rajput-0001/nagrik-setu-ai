...
LORA_PATH = os.getenv("LORA_PATH", "./lora")

app = FastAPI(title="Nagrik Setu AI", version="1.0")
...
def classify_complaint(complaint: Complaint):
    ...
    prompt = f"""Classify this civic complaint.
...
"""
    ...
    messages = [
        {"role": "system", "content": "You are Nagrik Setu, an AI civic complaint classifier. Return only JSON."},
    ...
    if start == -1 or end == -1:
        raise HTTPException(
            status_code=500,
            detail={"message": "AI did not return valid JSON", "raw_output": result_text},
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"message": "AI returned invalid JSON", "raw_output": result_text},
        ) from exc
